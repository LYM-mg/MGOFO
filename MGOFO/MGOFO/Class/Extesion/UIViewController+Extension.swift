...
extension UIViewController {
     ...
     *  @param hint 提示
     */
    func showHudInView(view: UIView, hint: String?, yOffset: CGFloat = 0.0) {
    ...
    func showHint(hint: String?,mode: MBProgressHUDMode? = .text,view: UIView? = (UIApplication.shared.delegate?.window!)!, yOffset: CGFloat = 0.0){
        //显示提示信息
        let hud = MBProgressHUD.showAdded(to: view!, animated: true)
        hud.isUserInteractionEnabled = false
        hud.mode = MBProgressHUDMode.text
    ...
    func mg_showWebGifLoading(frame:CGRect?, gifName: String?,view: UIView?, filter: Bool? = false) {
        ...
        if frame == CGRect.zero || frame == nil {
            frame = CGRect(x: 0, y: (self.navigationController != nil) ? navHeight : CGFloat(0), width: mainScreenW, height: mainScreenH)
        }
        ...
        DispatchQueue.global().async {
            ...
            if (frame?.size.height)! < mainScreenH - navHeight {
